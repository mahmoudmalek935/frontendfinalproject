...
const initialCategories = [
  { id: 1, title: "Plumbing", description: "Pipe repairs, leak fixes, installations, and drainage solutions.", providers: 24, icon: Wrench, iconBg: "bg-cyan-100", iconColor: "text-cyan-600" },
  { id: 2, title: "Electrical", description: "Wiring, lighting, panel upgrades, and electrical maintenance.", providers: 18, icon: Zap, iconBg: "bg-amber-100", iconColor: "text-amber-500" },
  { id: 3, title: "Cleaning", description: "Deep cleaning, regular housekeeping, and sanitization services.", providers: 42, icon: Sparkles, iconBg: "bg-cyan-100", iconColor: "text-cyan-600" },
  { id: 4, title: "Painting", description: "Interior and exterior painting, wall finishes, and touch-ups.", providers: 15, icon: PaintRoller, iconBg: "bg-amber-100", iconColor: "text-amber-500" },
  { id: 5, title: "Carpentry", description: "Furniture assembly, repairs, custom woodwork, and fittings.", providers: 11, icon: Hammer, iconBg: "bg-cyan-100", iconColor: "text-cyan-600" },
  { id: 6, title: "AC & Cooling", description: "Air conditioning installation, servicing, and repairs.", providers: 20, icon: Snowflake, iconBg: "bg-amber-100", iconColor: "text-amber-500" },
  { id: 7, title: "Gardening", description: "Landscaping, lawn care, plant maintenance, and garden design.", providers: 9, icon: Leaf, iconBg: "bg-cyan-100", iconColor: "text-cyan-600" },
  { id: 8, title: "Pest Control", description: "Inspection, extermination, and prevention treatments.", providers: 7, icon: Bug, iconBg: "bg-amber-100", iconColor: "text-amber-500" },
]

export default function ManageCategories() {
  const navigate = useNavigate();
  
  // 🔴 حولنا الداتا لـ State عشان نقدر نعدل ونمسح ونضيف 🔴
  const [categories, setCategories] = useState(initialCategories);

  // إضافة قسم جديد (استخدمنا Prompt كبديل سريع للـ Modal)
  const handleAddCategory = () => {
    const newTitle = window.prompt("Enter the name of the new category:");
    if (newTitle && newTitle.trim() !== "") {
      const newCategory = {
        id: Date.now(), // ID وهمي
        title: newTitle,
        description: "New category added by Admin. Description goes here.",
        providers: 0,
        icon: Wrench, // أيكونة افتراضية
        iconBg: "bg-slate-100",
  ...
  const handleEdit = (id, oldTitle) => {
    const newTitle = window.prompt("Edit category name:", oldTitle);
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {categories.map((category) => {
            ...
            return (
              ...
              >
                <div className={`flex h-16 w-16 items-center justify-center rounded-2xl ${category.iconBg}`}>
